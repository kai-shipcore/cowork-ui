...
export function VehicleResearchDetailPage() {
  ...
  const previewAsset = materials.find((item) => item.id === previewAssetId);
  ...
  return (
    <section className="research-detail-page">
                      ...
                      {group.rows.map((target) => {
                        ...
                        return (
                          <article
                            ...
                            data-focused={focusedConfigurationId === target.id}
                            aria-current={
                          ...
                          >
                            <button
                              ...
                              onClick={() => {
                                void navigate(
                                  `/vehicle-research/${encodeURIComponent(target.configurationId)}/configurations/${encodeURIComponent(target.id)}`,
                                );
                              }}
                            >
                  ...
                  >
                    {assetPageLoading
                      ...
                      : assetPageItems.map((item) => {
                          ...
                          const itemProductTypeIds = item.productTypeIds.length
                            ? item.productTypeIds
                            : [item.productTypeId];
                          const optionGroups = itemProductTypeIds.map(
                            (productTypeId) => ({
                              productTypeId,
                              productLabel:
                                PRODUCT_TYPES.find(
                              ...
                              options: item.selectedOptionValues.length
                                ? item.selectedOptionValues
                                    .filter(
                                      (option) =>
                                        option.productTypeId === productTypeId,
                                    )
                                    .map(
                                      (option) =>
                                        [option.key, option.value] as const,
                                    )
                                : item.optionSelections.filter(([key]) =>
                                    vehicleOptionKeys.some(
                                      (optionKey) =>
                                        optionKey.productTypeId ===
                                          productTypeId &&
                                        optionKey.name === key,
                                    ),
                                  ),
                          ...
                          return (
                            <article key={item.id}>
                              <button
                                type="button"
                                className="research-asset-mapping-preview"
                                aria-label={`Preview ${item.fileName || item.title}`}
                                onClick={() => {
                              ...
                              <div className="research-asset-mapping-body">
                                ...
                                <div className="research-asset-assignment">
                                  <div className="research-asset-assignment-summary">
                                    {item.years.length > 0 && (
                                      <span
                                        data-matched={
                                          selectedAssetCandidate?.years.some(
                                  ...
                                  <div className="research-asset-option-groups">
                                    {optionGroups.map((group) => (
                                      <section key={group.productTypeId}>
                                        <strong
                                          data-matched={
                                            selectedAssetCandidate?.productTypeId ===
                                              group.productTypeId || undefined
                                          }
                                        >
                                          {group.productLabel}
                                        </strong>
                                        <div className="research-asset-tag-list">
                                          {group.options.length > 0 ? (
                                              ...
                                              ([key, value]) => (
                                                <span
                                                  ...
                                                  data-matched={
                                                    (selectedAssetCandidate?.productTypeId ===
                                                      group.productTypeId &&
                                                      selectedAssetCandidate.options.some(
                                                        ([
                                                          candidateKey,
                                                          candidateValue,
                                                        ]) =>
                                            ...
                                            <small>No option values</small>
                                          )}
                                        </div>
                                      </section>
                                    ))}
                                  </div>
                                  <div className="research-asset-zone-group">
                                    <strong>Vehicle zones</strong>
                                    <div className="research-asset-tag-list">
                                      {item.vehicleZoneIds.map((zoneId) => (
                                        <span
                                          key={zoneId}
                                          data-matched={
                                            selectedAssetCandidate?.zoneIds.includes(
                                              zoneId,
                                            )
                                              ? true
                                              : undefined
                                  ...
                                  {editingAssetId === item.id ? (
                                    <>
                                      ...
                                      <Button
                                        ...
                                        onClick={() =>
                                          void saveAssetClassification(item.id)
                                        }
                                      >
                                        <Save /> Save
                                      </Button>
      ...
      >
        <DialogContent className="research-asset-preview-dialog">
          <DialogHeader>
            <DialogTitle>
              {previewAsset?.fileName ?? previewAsset?.title ?? 'Asset preview'}
            </DialogTitle>
            <p>Research asset preview and saved classification.</p>
          </DialogHeader>
          {previewAsset && (
            <DialogBody className="research-asset-preview-body">
              <div className="research-asset-preview-media">
                {previewAsset.fileData ? (
                  ...
                  <div>
                    <strong>LINK</strong>
                    <span>No uploaded image preview is available.</span>
              ...
              <dl className="research-asset-preview-metadata">
                <div>
                  <dt>Year</dt>
                  <dd>
                ...
                <div>
                  <dt>Product types</dt>
                  <dd>
                    {(previewAsset.productTypeIds.length
                      ? previewAsset.productTypeIds
                      : [previewAsset.productTypeId]
                    )
                      .map(

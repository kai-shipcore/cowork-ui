...
export function HuntWorkList({
  ...
  return (
    <div className="hunt-work-list">
      ...
      >
        <TabsList aria-label={`${label} 처리 상태`}>
        ...
        <TabsContent value={status} className="hunt-work-list">
          <div
            className="hunt-list-filters"
            aria-label={`${label} ${done ? '완료' : '대기'} 검색 및 필터`}
          >
            <label className="hunt-list-search">
              {label} {done ? '완료' : '대기'} 검색
              <Input
            ...
            {done ? (
              ...
              <label>
                ...
                >
                  ...
                  <option value="no">미예약</option>
                  <option value="yes">예약됨</option>
                </select>
              </label>
            )}
          ...
          {invalidRange && (
            <p role="alert">종료일은 시작일 이후로 선택하세요.</p>
          )}
          <div className="hunt-list-summary">
            <h2>
              {label} {done ? '완료' : '대기'}
            </h2>
            <span aria-live="polite">
              {filtered.length} / {subset.length}건
            </span>
            <small>
              {done
                ? '최근 완료 방문순 · 날짜 미기록은 마지막에 표시'
                : '미예약 우선 · 예약일순'}
            </small>
          </div>
          {done && (
            <p className="muted-text">
              날짜는 마지막 완료 방문일입니다. 방문 기록 없이 완료된 항목은
              ‘미기록’으로 표시됩니다.
            </p>
          )}
          <div className="hunt-list-table">
            <Table>
              <TableHeader>
                <TableRow>
            ...
            onPaginationChange={(action) =>
              setFilters((current) => ({
                ...
                [status]: {
                  ...current[status],
                  page:
                    typeof action === 'function' ? action(pagination) : action,
                },
              }))
            }
          />

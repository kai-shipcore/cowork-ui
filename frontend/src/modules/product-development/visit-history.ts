...
export function defaultVisitType(
  product: ProductType,
  stage: ProjectStage,
): ProjectVisit['type'] {
  return product === 'Car Cover' || stage === 'Fitting' || stage === 'Approved'
    ? 'FITTING'
    : 'SCAN';
}
...
export function visitTimestamp(visit: ProjectVisit, completed = false): number {
  const value = Date.parse(
    completed && visit.performedAt
      ? visit.performedAt
      : `${visit.date}T${visit.time}`,
  );
...
export function visitHistory(
  ...
  const completed = selected
    ...
    .sort((a, b) => visitTimestamp(a, true) - visitTimestamp(b, true));
  return {
    upcoming: selected
    ...
    past: selected
      ...
      .sort((a, b) => visitTimestamp(b, true) - visitTimestamp(a, true)),
    latest: completed.slice(-1).pop(),

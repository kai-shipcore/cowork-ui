...
void test('failed and unreceived lines are not uninspected receipts', () => {
  ...
  assert.equal(
    matchesInspectionRequest(
      [{ ...received, sampleReceivedAt: undefined }],
      'ARRIVED',
    ),
    false,
  );
...
void test('pending part lines exclude passed and failed siblings', () => {
  assert.deepEqual(
    [received, passed, failed].filter((i) =>
      matchesInspectionItem(i, 'ARRIVED'),
    ),
    [received],
  );
  assert.equal(
    matchesInspectionRequest([{ ...passed, sampleRound: 2 }], 'PASSED'),
    false,
  );
  assert.equal(
    matchesInspectionRequest(
      [{ ...passed, sampleRound: 2, revisionReflected: 'CORRECT' }],
      'PASSED',
    ),
    true,
  );
